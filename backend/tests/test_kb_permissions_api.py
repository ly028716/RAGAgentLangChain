...
class TestKnowledgeBasePermissionsAPI:
    ...
    def test_shared_user_can_get_kb_detail(
        self,
        client: TestClient,
        auth_headers: dict,
        ...
        resp = client.get(f"/api/v1/knowledge-bases/{test_kb.id}", headers=other_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert data["id"] == test_kb.id

    def test_shared_user_can_list_kbs(
        self,
        client: TestClient,
        auth_headers: dict,
        test_kb: KnowledgeBase,
        ...
        resp = client.get("/api/v1/knowledge-bases", headers=other_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert data["total"] >= 1
        assert any(item["id"] == test_kb.id for item in data["items"])
