...
class UserService:
    ...
    def request_deletion(
        self, user_id: int, password: str, reason: Optional[str] = None
    ) -> dict:
        """
        请求账号注销
    ...
    def cancel_deletion(self, user_id: int) -> dict:
        ...
        return {"success": True, "message": "注销请求已取消，您的账号将保持正常状态"}
    ...
    def get_deletion_status(self, user_id: int) -> dict:
        ...
        if user.deletion_requested_at is None:
            return {"has_deletion_request": False, "message": "您的账号状态正常，没有待处理的注销请求"}
        ...
        return {
            ...
            "message": f"您的账号将于 {remaining_days} 天 {remaining_hours} 小时后被删除"
            if can_cancel
            else "冷静期已过，账号即将被删除",
        }
