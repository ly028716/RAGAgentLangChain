...
class SystemService:
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            # 数据库配置
            "database": {
                "url": self._mask_sensitive_value(settings.database.database_url, show_chars=10),
            ...
            # 通义千问配置
            "tongyi": {
                "api_key": self._mask_sensitive_value(settings.tongyi.dashscope_api_key),
    ...
    def get_usage_stats(
        ...
        daily_stats = query.with_entities(
            ...
            func.count(APIUsage.id).label('call_count'),
            func.sum(APIUsage.tokens_used).label('total_tokens')
        ).group_by(func.date(APIUsage.created_at)).order_by('date').all()
        ...
        if user_id:
            user_quota = self.db.query(UserQuota).filter(
        ...
        stats = {
            ...
            "summary": {
                ...
                "average_tokens_per_call": int(total_tokens / total_calls) if total_calls > 0 else 0,
            },
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        today_usage = self.db.query(APIUsage).filter(
        ...
        return {
            ...
            "statistics": {
                ...
                "today_tokens_used": int(today_usage[1]) if today_usage and today_usage[1] else 0,
            },
            "uptime": {
                "started_at": getattr(self, '_start_time', datetime.utcnow()).isoformat(),
            }
        }
