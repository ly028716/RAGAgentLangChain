...
class FileService:
    ...
    async def validate_avatar(self, file: UploadFile) -> Tuple[bytes, str]:
        ...
        if file.content_type not in self.ALLOWED_IMAGE_TYPES:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="不支持的图片类型"
            )
        ...
        if not self._validate_image_content(content):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="文件内容无效，请上传真实的图片文件"
            )
    ...
    async def save_avatar(self, user_id: int, file: UploadFile) -> dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="保存头像失败"
            )
