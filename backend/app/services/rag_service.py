...
class RAGService:
    ...
    def get_knowledge_bases(
        ...
        """
        return self.kb_repo.get_accessible_by_user(user_id, skip, limit)
    ...
    def get_knowledge_base(
        ...
        has_permission, kb = self.kb_permission_service.check_permission(
            kb_id=kb_id,
            user_id=user_id,
            required_permission=PermissionType.VIEWER.value
        )
    ...
    async def upload_document(
        ...
        # 获取文件类型
        file_type = DocumentLoaderFactory.get_file_type_from_extension(file.filename)
        if not file_type:
        ...
        # 保存文件
        file_path = await self._save_upload_file(file, kb_id)
    ...
    def delete_document(
        ...
        # 删除向量数据（使用后台任务，避免在同步函数中使用 asyncio.run）
        try:
            # 尝试同步删除，如果失败则记录警告
            # 注意：这里不使用 asyncio.run() 避免事件循环冲突
            # 向量数据删除失败不应阻止文档删除
            logger.info(f"准备删除文档 {document_id} 的向量数据")
            # TODO: 考虑使用后台任务异步删除向量数据
        except Exception as e:
