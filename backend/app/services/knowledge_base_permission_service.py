...
class KnowledgeBasePermissionService:
    ...
    def check_permission(
        ...
        # 检查用户权限
        permission = self.db.query(KnowledgeBasePermission).filter(
    ...
    def get_permissions(
        ...
        """
        # 检查是否有权限查看
        has_permission, kb = self.check_permission(kb_id, user_id, PermissionType.OWNER.value)
        ...
        permissions = self.db.query(KnowledgeBasePermission).options(
            joinedload(KnowledgeBasePermission.user)
        ).filter(
            KnowledgeBasePermission.knowledge_base_id == kb_id
        ).all()
    ...
    def add_permission(
        ...
        # 检查是否已存在权限
        existing = self.db.query(KnowledgeBasePermission).filter(
            KnowledgeBasePermission.knowledge_base_id == kb_id,
        ...
        logger.info(f"添加权限成功: kb_id={kb_id}, user_id={data.user_id}, type={data.permission_type}")
        return permission
    ...
    def update_permission(
        ...
        logger.info(f"更新权限成功: permission_id={permission_id}, type={data.permission_type}")
        return permission
    ...
    def get_shared_knowledge_bases(
        ...
        """
        # 查询用户有权限的知识库（非自己创建的）
        query = self.db.query(KnowledgeBase).join(
