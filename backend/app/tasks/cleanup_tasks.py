...
def run_all_cleanup_tasks() -> dict:
    ...
    results = {"start_time": start_time.isoformat(), "tasks": {}}
    ...
    try:
        ...
        results["tasks"]["login_attempts"] = {"success": False, "error": str(e)}
    ...
    try:
        ...
        results["tasks"]["temp_files"] = {"success": False, "error": str(e)}
    ...
    try:
        ...
        results["tasks"]["api_usage"] = {"success": False, "error": str(e)}
    ...
    try:
        ...
        results["tasks"]["account_deletions"] = {"success": False, "error": str(e)}
    ...
    # 统计成功的任务数
    success_count = sum(
        1 for task in results["tasks"].values() if task.get("success", False)
    )
    ...
    logger.info(f"所有清理任务完成: {success_count}/{total_count} 成功, " f"耗时 {duration:.2f} 秒")
        ...
        for user in users_to_delete:
            try:
                ...
                if result.get("success"):
                    ...
                    logger.error(
                        f"删除用户 {user.username} (ID: {user.id}) 失败: {result.get('message')}"
                    )
