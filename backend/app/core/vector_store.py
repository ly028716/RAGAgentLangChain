...
class VectorStoreManager:
    ...
    def get_vector_store(self, knowledge_base_id: int) -> Chroma:
        ...
        if knowledge_base_id not in self._vector_stores:
            self._vector_stores[knowledge_base_id] = self._create_vector_store(knowledge_base_id)
    ...
    async def similarity_search(
        ...
        logger.debug(
            f"相似度搜索: kb_id={knowledge_base_id}, "
    ...
    async def similarity_search_with_score(
        ...
        logger.debug(
            f"带评分相似度搜索: kb_id={knowledge_base_id}, "
            f"query长度={len(query)}, k={k}"
        )
    ...
    async def delete_by_document_id(
        ...
        logger.info(
            f"删除文档向量: kb_id={knowledge_base_id}, "
