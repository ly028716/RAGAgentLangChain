...
class ConnectionManager:
    ...
    def disconnect(self, user_id: int):
        ...
        if user_id in self.connection_times:
            connection_duration = (datetime.utcnow() - self.connection_times[user_id]).total_seconds()
            del self.connection_times[user_id]
            logger.info(f"WebSocket连接已断开 user_id={user_id}, 持续时间={connection_duration:.2f}秒")
        else:
    ...
    async def broadcast(self, message: dict, exclude_user_ids: Optional[list] = None):
        ...
        logger.info(f"消息已广播给 {len(self.active_connections) - len(exclude_user_ids)} 个用户")
