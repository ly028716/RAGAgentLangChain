...
class APIUsage(Base):
    ...
    # 使用信息
    api_type = Column(
        String(50), nullable=False, index=True, comment="API类型（chat/rag/agent等）"
    )
    tokens_used = Column(Integer, nullable=False, comment="消耗的token数量")
    cost = Column(
        Numeric(10, 4), default=Decimal("0.0000"), nullable=False, comment="调用费用"
    )

    # 时间戳
    created_at = Column(
        DateTime, default=datetime.utcnow, nullable=False, index=True, comment="调用时间"
    )
