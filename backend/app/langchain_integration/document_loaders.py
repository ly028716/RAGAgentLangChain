        ...
        try:
            ...
            if e.name == "docx2txt":
                raise DocumentProcessingError(
                    "DOCX解析依赖缺失：docx2txt。请安装 docx2txt 后重试。"
                ) from e
            raise


def _decode_bytes_fallback(content: bytes, encodings: List[str]) -> str:
...
class RobustMarkdownLoader(BaseLoader):
    def __init__(self, file_path: str):
        self.file_path = file_path

    def load(self) -> List[Document]:
        try:
            return UnstructuredMarkdownLoader(self.file_path).load()
        except Exception as e:
            logger.warning(f"Markdown结构化解析失败，降级为纯文本加载: error={str(e)}")
            return RobustTextLoader(self.file_path).load()
...
class DocumentLoaderFactory:
    ...
    SUPPORTED_LOADERS = {
        "pdf": PyPDFLoader,
        "docx": DocxLoader,
        "txt": RobustTextLoader,
        "md": RobustMarkdownLoader,
        "markdown": RobustMarkdownLoader,
    }
    ...
    def get_loader(cls, file_path: str, file_type: str) -> BaseLoader:
        ...
        if not loader_class:
            ...
            raise UnsupportedFileTypeError(f"不支持的文件类型: {file_type}。支持的类型: {supported}")
    ...
    def get_document_preview(
        ...
        """
        try:
            preview, _ = cls.get_document_preview_with_length(
                file_path=file_path,
                file_type=file_type,
                max_chars=max_chars,
            )
