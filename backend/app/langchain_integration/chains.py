...
class ChatConfig:
    ...
    def __init__(
        self, temperature: float = 0.7, max_tokens: int = 2000, mode: str = "normal"
    ):
        """
        初始化对话配置
...
class ConversationManager:
    ...
    def __init__(self):
        ...
        self._prompt_template = PromptTemplate(
            input_variables=["history", "input"], template=DEFAULT_CONVERSATION_TEMPLATE
        )

    def get_or_create_memory(
        self, conversation_id: int, history: Optional[List[Dict[str, str]]] = None
        ...
        if conversation_id not in self._memories:
            memory = ConversationBufferMemory(
                return_messages=True, human_prefix="用户", ai_prefix="AI助手"
            )
    ...
    def load_history(
        self, conversation_id: int, messages: List[Dict[str, str]]
    ) -> None:
        """
        加载对话历史到记忆
    ...
    def _get_llm(self, config: ChatConfig, streaming: bool = False) -> TongyiLLM:
        """
        根据配置获取LLM实例
        ...
        return get_llm(temperature=config.temperature, max_tokens=config.max_tokens)

    def _build_prompt(self, message: str, memory: ConversationBufferMemory) -> str:
        """
        构建对话提示
        ...
        return self._prompt_template.format(history=history_str.strip(), input=message)
    ...
    async def chat(
        ...
        try:
            ...
            logger.debug(
                f"对话 {conversation_id}: 收到回复，长度={len(response)}, tokens≈{estimated_tokens}"
            )
    ...
    async def stream_chat(
        ...
        try:
            ...
            async def _emit_text(text: Any) -> AsyncGenerator[Dict[str, Any], None]:
                nonlocal full_response, yielded_any
                if text is None:
                    return
            ...
            async def _stream_via_private_astream() -> AsyncGenerator[Dict[str, Any], None]:
                async for chunk in llm.llm._astream(prompt):
            ...
            async def _stream_via_public_astream() -> AsyncGenerator[Dict[str, Any], None]:
            ...
            async def _stream_via_invoke() -> AsyncGenerator[Dict[str, Any], None]:
                ainvoke = getattr(llm.llm, "ainvoke", None)
                invoke = getattr(llm.llm, "invoke", None)
                if callable(ainvoke):
                    text = await ainvoke(prompt)
                elif callable(invoke):
                    text = invoke(prompt)
                else:
                    raise TypeError("LLM does not support invoke/ainvoke")
                async for event in _emit_text(text):
                    yield event

            astream_impl = getattr(llm.llm, "_astream", None)
            try:
                if callable(astream_impl):
                    async for event in _stream_via_private_astream():
                        yield event
                else:
                    async for event in _stream_via_public_astream():
                        yield event
            except Exception:
                if yielded_any:
                    raise
                async for event in _stream_via_invoke():
                    yield event
            ...
            logger.debug(
                f"对话 {conversation_id}: 流式对话完成，总长度={len(full_response)}, tokens≈{estimated_tokens}"
            )
            ...
            logger.exception(f"对话 {conversation_id}: 流式LLM调用失败 - {str(e)}")
    ...
    def get_memory_messages(self, conversation_id: int) -> List[Dict[str, str]]:
        """
        获取对话记忆中的消息
