...
class JWTSettings(BaseSettings):
    ...
    access_token_expire_days: int = Field(default=7, ge=1, le=365, description="访问令牌有效期（天）")
    refresh_token_expire_days: int = Field(default=30, ge=1, le=365, description="刷新令牌有效期（天）")
...
class SecuritySettings(BaseSettings):
    ...
    account_lockout_minutes: int = Field(default=15, ge=5, le=60, description="账户锁定时长（分钟）")
...
class TongyiSettings(BaseSettings):
    ...
    def validate_api_key(cls, v: str) -> str:
        if v == "your-dashscope-api-key-here":
...
class FileStorageSettings(BaseSettings):
    ...
    max_upload_size_mb: int = Field(default=10, ge=1, le=100, description="最大上传文件大小（MB）")
...
class RAGSettings(BaseSettings):
    ...
    rag_similarity_threshold: float = Field(default=0.7, ge=0.0, le=1.0, description="相似度阈值")
...
class QuotaSettings(BaseSettings):
    ...
    default_monthly_quota: int = Field(default=100000, ge=1000, description="默认月度配额（tokens）")
...
class RateLimitSettings(BaseSettings):
    ...
    rate_limit_login_per_minute: int = Field(default=5, ge=1, description="登录API每分钟请求限制")
...
class Settings:
    ...
    def validate_all(self) -> bool:
        ...
        try:
            ...
            assert self.tongyi.dashscope_api_key != "your-dashscope-api-key-here", "DashScope API密钥未配置"
    ...
    def get_config_summary(self) -> dict:
        ...
        return {
            ...
            "tongyi_configured": self.tongyi.dashscope_api_key != "your-dashscope-api-key-here",
