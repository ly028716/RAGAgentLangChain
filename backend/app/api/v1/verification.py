...
from app.schemas.verification import (SendCodeResponse, SendEmailCodeRequest,
                                      SendSMSCodeRequest, VerifyCodeRequest,
                                      VerifyCodeResponse)
...
async def send_email_code(
    request: SendEmailCodeRequest, db: Session = Depends(get_db)
) -> SendCodeResponse:
    """
    发送邮箱验证码
    ...
    result = await service.send_email_code(
        email=request.email, code_type=request.code_type
    )
...
async def send_sms_code(
    request: SendSMSCodeRequest, db: Session = Depends(get_db)
) -> SendCodeResponse:
    """
    发送短信验证码
    ...
    result = await service.send_sms_code(
        phone=request.phone, code_type=request.code_type
    )
...
def verify_code(
    ...
    result = service.verify_code(
        target=request.target, code=request.code, code_type=request.code_type
    )
