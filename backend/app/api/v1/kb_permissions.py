...
from app.services.knowledge_base_permission_service import KnowledgeBasePermissionService
...
def get_permissions(
    ...
    return PermissionListResponse(
        items=[PermissionResponse(**p) for p in permissions],
        total=total
    )
...
def add_permission(
    ...
    # 获取用户名
    user = db.query(User).filter(User.id == permission.user_id).first() if permission.user_id else None
