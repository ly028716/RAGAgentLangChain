...
async def update_quota(
    ...
    try:
        quota = quota_service.update_quota(
            ...
            new_quota=request.monthly_quota
        )
        
        logger.info(f"管理员 {current_user.username} 更新用户 {request.user_id} 的配额为 {request.monthly_quota}")
        ...
        return QuotaUpdateResponse(
            ...
            updated_at=quota.updated_at.isoformat() if quota.updated_at else datetime.utcnow().isoformat()
        )
