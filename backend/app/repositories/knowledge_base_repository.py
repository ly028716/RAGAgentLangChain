...
class KnowledgeBaseRepository:
    ...
    def get_by_id_and_user(self, kb_id: int, user_id: int) -> Optional[KnowledgeBase]:
        """
        根据ID和用户ID获取知识库
    ...
    def get_by_user(
        ...
        """
        query = self.db.query(KnowledgeBase).filter(KnowledgeBase.user_id == user_id)
        ...
        knowledge_bases = (
            query.options(joinedload(KnowledgeBase.documents))
    ...
    def get_by_category(
        self, user_id: int, category: str, skip: int = 0, limit: int = 20
    ) -> Tuple[List[KnowledgeBase], int]:
        """
        根据分类获取用户的知识库列表（分页）
        ...
        query = self.db.query(KnowledgeBase).filter(
            KnowledgeBase.user_id == user_id, KnowledgeBase.category == category
        )

        total = query.count()

        knowledge_bases = (
            query.order_by(desc(KnowledgeBase.updated_at))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_document_count(self, kb_id: int) -> int:
        ...
        """
        return (
            self.db.query(Document).filter(Document.knowledge_base_id == kb_id).count()
        )

    def exists(self, kb_id: int, user_id: int) -> bool:
        """
        检查知识库是否存在且属于指定用户
    ...
    def search_by_name(
        self, user_id: int, keyword: str, skip: int = 0, limit: int = 20
    ) -> Tuple[List[KnowledgeBase], int]:
        """
        根据名称关键字搜索知识库
        ...
        """
        query = self.db.query(KnowledgeBase).filter(
            KnowledgeBase.user_id == user_id, KnowledgeBase.name.ilike(f"%{keyword}%")
        )
