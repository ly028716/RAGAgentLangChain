...
class MessageRepository:
    ...
    def create(
        self, conversation_id: int, role: MessageRole, content: str, tokens: int = 0
    ) -> Message:
        """
        创建新消息
    ...
    def get_by_conversation_paginated(
        ...
        # 按创建时间升序排列并分页
        messages = (
            query.order_by(asc(Message.created_at)).offset(skip).limit(limit).all()
        )
    ...
    def get_first_user_message(self, conversation_id: int) -> Optional[Message]:
        """
        获取对话的第一条用户消息
    ...
    def update_tokens(self, message_id: int, tokens: int) -> Optional[Message]:
        """
        更新消息的token数量
    ...
    def get_total_tokens(self, conversation_id: int) -> int:
        ...
        """
        result = (
            self.db.query(func.sum(Message.tokens))
            .filter(Message.conversation_id == conversation_id)
            .scalar()
        )
        return result or 0

    def get_tokens_by_role(self, conversation_id: int, role: MessageRole) -> int:
        """
        获取对话中指定角色的总token消耗
    ...
    def get_messages_by_role(
        self, conversation_id: int, role: MessageRole
    ) -> List[Message]:
        """
        获取对话中指定角色的所有消息
    ...
    def exists(self, message_id: int) -> bool:
        ...
        """
        return (
            self.db.query(Message).filter(Message.id == message_id).first() is not None
        )
