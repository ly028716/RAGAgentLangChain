...
class DocumentRepository:
    ...
    def get_by_knowledge_base(
        self, knowledge_base_id: int, skip: int = 0, limit: int = 20
    ...
    def get_all_by_knowledge_base(self, knowledge_base_id: int) -> List[Document]:
        """
        获取知识库的所有文档（不分页）
    ...
    def get_by_status(
        ...
        """
        query = self.db.query(Document).filter(
            Document.knowledge_base_id == knowledge_base_id, Document.status == status
        )
    ...
    def mark_completed(self, document_id: int, chunk_count: int) -> Optional[Document]:
        """
        标记文档处理完成
        ...
        return self.update_status(
            document_id, DocumentStatus.COMPLETED, chunk_count=chunk_count
        )

    def mark_failed(self, document_id: int, error_message: str) -> Optional[Document]:
        """
        标记文档处理失败
    ...
    def delete_by_kb(self, document_id: int, knowledge_base_id: int) -> bool:
        """
        删除指定知识库中的文档
    ...
    def count_by_status(self, knowledge_base_id: int, status: DocumentStatus) -> int:
        """
        获取知识库中指定状态的文档数量
    ...
    def exists(self, document_id: int, knowledge_base_id: int) -> bool:
        """
        检查文档是否存在且属于指定知识库
    ...
    def get_completed_documents(self, knowledge_base_id: int) -> List[Document]:
        """
        获取知识库中所有已完成处理的文档
    ...
    def get_by_filename(
        self, knowledge_base_id: int, filename: str
    ) -> Optional[Document]:
        """
        根据文件名获取文档
